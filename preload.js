...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("launchTradingView")
...
const executeAutomation = async (stocks, username, password, website) => {
  if (!stocks) {
    return;
  }
...
const loginIntoTradeView = async (browser, username, password) => {
  const userButton = await browser.$("//*[@aria-label='Open user menu']");
  await userButton.click();
  await browser.pause(1000);

  const darkModeButton = await browser.$(
    "//*[@data-name='header-user-menu-switch-theme']"
  );
  await darkModeButton.click();
  await browser.pause(1000);

  const signInButton = await browser.$(
...
const loginIntoScreener = async (browser, username, password) => {
